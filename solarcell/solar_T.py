...
smu.set_voltage(0)
smu.set_current_range(1E-1)
smu.set_current_limit(1E-1)
smu.set_current(0)
...
def sweep_voltage(temperature, smu):
    """ ******* For saving the data ******** """

    # Create unique filenames for saving the data
    time_for_name = datetime.datetime.now().strftime("%Y_%m_%d_%H%M%S")
    filename_csv = 'IV' + time_for_name + str(temperature) +'.csv'
    filename_pdf = 'IV' + time_for_name + str(temperature)+'.pdf'
...
# Create unique filenames for saving the data
time_for_name = datetime.datetime.now().strftime("%Y_%m_%d_%H%M%S")
filename_csv = 'PT' + time_for_name + str(temperature) +'.csv'
filename_pdf = 'PT' + time_for_name + str(temperature)+'.pdf'
...
plt.plot(temperature_array, max_power_array,'x-', linewidth=2)

# set labels and a title
plt.xlabel('Temperature / C', fontsize=14)
plt.ylabel('Max Power / W', fontsize=14)
plt.title('Max power temperature plot', fontsize=14)
